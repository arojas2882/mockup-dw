...
function genPrimariosRows(dates) {
  return dates.map(date => {
    const BCA = rnd(0,50), BOR = rnd(0,50), ACA = rnd(0,50), AOR = rnd(0,50);
    const NST = rnd(0,50), SR = rnd(0,50), TA = rnd(0,50), Nulos = rnd(0,10);
    return {
      date, BCA, BOR, ACA, AOR, NST, SR, TA, Nulos,
      totBasicas: BCA+BOR, totAvanzadas: ACA+AOR,
      totValidas: BCA+BOR+ACA+AOR+NST+SR+TA,
      totValidasNulos: BCA+BOR+ACA+AOR+NST+SR+TA+Nulos,
    };
  });
}
...
function genSecundariosRows(dates) {
  return dates.map(date => {
    const BCA = rnd(0,50), BOR = rnd(0,50), ACA = rnd(0,50), AOR = rnd(0,50);
    const NST = rnd(0,50), SR = rnd(0,50), TA = rnd(0,50), Nulos = rnd(0,8);
    const totValidasREM = BCA+BOR+ACA+AOR+NST+SR+TA;
    return { date, BCA, BOR, ACA, AOR, NST, SR, TA, Nulos, totFichasNulos: totValidasREM+Nulos, totValidasREM };
  });
}
...
function TablaPrimarios({ rows }) {
  const head = ["Fecha","Básicas Centro Asistencial","Básicas Otro Resultado","Avanzada Centro Asistencial","Avanzada Otro Resultado","Intervención Pacientes No Trasladados","Sin Registro","Traslado Aéreo","Tot. Básicas","Tot. Avanzadas","Tot. Fichas Válidas","Tot. Fichas + Nulos"];
  const csvRows = [head, ...rows.map(r => [fmtDMY(r.date), r.BCA, r.BOR, r.ACA, r.AOR, r.NST, r.SR, r.TA, r.totBasicas, r.totAvanzadas, r.totValidas, r.totValidasNulos])];
  const remJ = rows.reduce((s,r)=>s+r.BCA+r.BOR+r.ACA+r.AOR+r.NST+r.SR+r.TA,0);
      ...
      <div className="table-wrap scroll-x">
        <table>
          ...
          <tbody>{rows.length===0 ? <tr><td colSpan={head.length}>NA — sin datos para el período</td></tr> : rows.map(r=>(
            <tr key={r.date}><td>{fmtDMY(r.date)}</td><td>{r.BCA}</td><td>{r.BOR}</td><td>{r.ACA}</td><td>{r.AOR}</td><td>{r.NST}</td><td>{r.SR}</td><td>{r.TA}</td>
              <td>{r.totBasicas}</td><td>{r.totAvanzadas}</td><td>{r.totValidas}</td><td>{r.totValidasNulos}</td></tr>
          ))}</tbody>
        </table>
      </div>
...
function TablaSecundarios({ rows }) {
  const head = ["Fecha","Básicas Centro Asistencial","Básicas Otro Resultado","Avanzada Centro Asistencial","Avanzada Otro Resultado","Intervención Pacientes No Trasladados","Sin Registro","Traslado Aéreo","Nulos","Tot. Fichas + Nulos","Tot. Válidas (REM)"];
  const csvRows = [head, ...rows.map(r => [fmtDMY(r.date), r.BCA, r.BOR, r.ACA, r.AOR, r.NST, r.SR, r.TA, r.Nulos, r.totFichasNulos, r.totValidasREM])];
  return (
    <>
      ...
      <div className="table-wrap scroll-x">
        <table>
          <thead><tr>{head.map(h=><th key={h}>{h}</th>)}</tr></thead>
          <tbody>{rows.length===0 ? <tr><td colSpan={head.length}>NA — sin datos para el período</td></tr> : rows.map(r=>(
            <tr key={r.date}><td>{fmtDMY(r.date)}</td><td>{r.BCA}</td><td>{r.BOR}</td><td>{r.ACA}</td><td>{r.AOR}</td><td>{r.NST}</td><td>{r.SR}</td><td>{r.TA}</td><td>{r.Nulos}</td><td>{r.totFichasNulos}</td><td>{r.totValidasREM}</td></tr>
          ))}</tbody>
        </table>
      </div>
    </>
...
function ModificarDatos({ log }) {
  ...
  const [campo, setCampo] = useState("Básicas Centro Asistencial");
  const [nuevoValor, setNuevoValor] = useState("");
  const [resultado, setResultado] = useState(null);

  const camposPorTipo = {
    "Traslado Primario": ["Básicas Centro Asistencial","Básicas Otro Resultado","Avanzada Centro Asistencial","Avanzada Otro Resultado","Intervención Pacientes No Trasladados","Sin Registro","Traslado Aéreo"],
    "Traslado Secundario": ["Básicas Centro Asistencial","Básicas Otro Resultado","Avanzada Centro Asistencial","Avanzada Otro Resultado","Intervención Pacientes No Trasladados","Sin Registro","Traslado Aéreo","Nulos"],
...
function Dashboard() {
  ...
  return (
    <div>
      ...
      {tab === "prim" && <DashTraslados titulo="Traslados Primarios" tipo="primario" />}
      {tab === "sec" && <DashTraslados titulo="Traslados Secundarios" tipo="secundario" />}
      {tab === "buz" && <DashBuzon />}
    </div>
  );
}
...
function DashTraslados({ titulo, tipo }) {
  const isPrimario = tipo === "primario";
  const tipos = [
    { key: "BCA", label: "Básicas Centro Asistencial" },
    { key: "BOR", label: "Básicas Otro Resultado" },
    { key: "ACA", label: "Avanzada Centro Asistencial" },
    { key: "AOR", label: "Avanzada Otro Resultado" },
    { key: "NST", label: "Intervención Pacientes No Trasladados" },
    { key: "SR", label: "Sin Registro" },
    { key: "TA", label: "Traslado Aéreo" },
  ];
  const [sel, setSel] = useState("BCA");
  const [yearA, setYearA] = useState(2025);
  const [yearB, setYearB] = useState(2026);
  const colores = ["#142347","#E8A800","#1E8E5A","#C0362C","#5B6EE8","#8A5BE8","#8B5A2B"];
  const evol = useMemo(() => Array.from({ length: 12 }, (_, i) => {
    const row = { mes: MONTHS[i].slice(0,3) };
    tipos.forEach(t => row[t.key] = rnd(100,900));
    row.totalBasicas = row.BCA + row.BOR;
    row.totalAvanzadas = row.ACA + row.AOR;
    row.totalValidas = tipos.reduce((s,t)=>s+row[t.key],0);
    row.totalNulos = row.totalValidas + rnd(0,80);
    return row;
  }), [titulo]);
  ...
  const selectedLabel = tipos.find(t => t.key === sel)?.label || sel;
      ...
      <div className="card">
        ...
        <ResponsiveContainer width="100%" height={240}>
          <LineChart data={evol}>
            ...
            <RTooltip />
            <Legend />
            <Line type="monotone" dataKey={sel} name={selectedLabel} stroke="#142347" strokeWidth={2.5} dot={{r:3}} />
          </LineChart>
        </ResponsiveContainer>
      </div>

      <div className="card">
        <div className="disp" style={{ fontSize: 15, fontWeight: 700, marginBottom: 8 }}>{titulo} — totales mensuales</div>
        <ResponsiveContainer width="100%" height={220}>
          <LineChart data={evol}>
            <CartesianGrid strokeDasharray="3 3" stroke="#E2E6F0" />
            <XAxis dataKey="mes" fontSize={11} /><YAxis fontSize={11} />
            <RTooltip /><Legend />
            {isPrimario ? <>
              <Line type="monotone" dataKey="totalBasicas" name="Totales Básicas" stroke="#142347" strokeWidth={2} dot={{r:3}} />
              <Line type="monotone" dataKey="totalAvanzadas" name="Totales Avanzadas" stroke="#E8A800" strokeWidth={2} dot={{r:3}} />
              <Line type="monotone" dataKey="totalValidas" name="Total fichas Válidas" stroke="#1E8E5A" strokeWidth={2} dot={{r:3}} />
              <Line type="monotone" dataKey="totalNulos" name="Total fichas + Nulos" stroke="#C0362C" strokeWidth={2} dot={{r:3}} />
            </> : <>
              <Line type="monotone" dataKey="totalValidas" name="Totales Válidas (REM)" stroke="#1E8E5A" strokeWidth={2} dot={{r:3}} />
              <Line type="monotone" dataKey="totalNulos" name="Totales Fichas + Nulos" stroke="#C0362C" strokeWidth={2} dot={{r:3}} />
            </>}
          </LineChart>
        </ResponsiveContainer>
      </div>
      ...
      <div className="card">
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 12, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <select className="select" style={{ width: 120 }} value={yearA} onChange={e=>setYearA(+e.target.value)}>
              {[2025,2026].map(y=><option key={y} value={y}>{y}</option>)}
            </select>
            <select className="select" style={{ width: 120 }} value={yearB} onChange={e=>setYearB(+e.target.value)}>
              {[2025,2026].map(y=><option key={y} value={y}>{y}</option>)}
            </select>
            <select className="select" style={{ width: 260 }} value={sel} onChange={e=>setSel(e.target.value)}>
              <option value="todos">Todos los tipos</option>
              {tipos.map(t=><option key={t.key} value={t.key}>{t.label}</option>)}
            </select>
          </div>
        </div>
        <ExportBar rows={[["Mes",String(yearA),String(yearB),"% cambio"], ...comparativo.map(c=>[c.mes,c.anterior,c.actual,(isFinite(c.pct)?c.pct+"%":"NA")])]} filenameBase={`comparativo_${titulo.replace(/\s/g,"_")}`} />
        <div className="table-wrap scroll-x">
          <table>
            <thead><tr><th>Mes</th><th>{yearA}</th><th>{yearB}</th><th>% cambio</th></tr></thead>
